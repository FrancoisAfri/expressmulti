                ...
                <div class="card-box">
                            ...
                            <div class="table-responsive">
                                <table class="table mt-4 table-centered">
                                    ...
                                    <tr>
                                        ...
                                        <td>R {{ $deleted ?? 0.00 }}</td>
                                        <td>R {{ number_format($eft - ($eft*  0.14) , 2, ',', '.') }}</td>
                                        <td>R {{ number_format( ($eft*  0.14) , 2, ',', '.') }}</td>
                                        <td>R {{ number_format($eft - ($eft*  0.14) , 2, ',', '.') }}</td>
                                    </tr>

                                    <tr>
                                        <td>{{ 'Payment :Debit Card -Patient' ?? '' }}</td>
                                        <td> R {{ number_format($Debit_Card  , 2, ',', '.') }} </td>
                                        <td>R {{ $deleted ?? 0.00 }}</td>
                                        <td>R {{ number_format($Debit_Card - ($Debit_Card*  0.14) , 2, ',', '.') }}</td>
                                        <td>R {{ number_format( ($Debit_Card*  0.14) , 2, ',', '.') }}</td>
                                        <td>R {{ number_format($Debit_Card - ($Debit_Card*  0.14) , 2, ',', '.') }}</td>
                                    </tr>

                                    <tr>
                                        <td>{{ 'Payment :Eft - Medical Scheme' ?? '' }}</td>
                                        <td> R {{ number_format($medical_scheme  , 2, ',', '.') }} </td>
                                        <td>R {{ $deleted ?? 0.00 }}</td>
                                        <td>R {{ number_format($medical_scheme - ($medical_scheme*  0.14) , 2, ',', '.') }}</td>
                                        <td>R {{ number_format( ($medical_scheme*  0.14) , 2, ',', '.') }}</td>
                                        <td>R {{ number_format($medical_scheme - ($medical_scheme*  0.14) , 2, ',', '.') }}</td>
                                    </tr>

                                    <tr>
                                      <td></td>
                                     <td>R {{ number_format($cash + $eft +$Debit_Card + $medical_scheme  , 2, ',', '.') ?? 0.00}}</td>
                                      <td></td>
                                        <td>R {{ number_format($cash - ($cash *  0.14) + $eft - ($eft *  0.14) +
                                            $Debit_Card - ($Debit_Card *  0.14) +
                                             $medical_scheme - ($medical_scheme *  0.14)  , 2, ',', '.') ?? 0.00}}
                                        </td>

                                        <td>R {{ number_format(($cash *  0.14) +  ($eft *  0.14) +
                                            ($Debit_Card *  0.14) + ($medical_scheme *  0.14), 2, ',', '.')  ?? 0.00}}
                                        </td>
                    ...
                    <div class="col-8">
                        <div class="table-responsive">
                            <table class="table mt-4 table-centered">
                                <thead>
                                <tr>
                                    <th>Invoices Base Vat Summary</th>
                                  <th></th>
                                  <th></th>
                                  <th></th>
                                  <th></th>
                    ...
                    <div class="mt-4 mb-1">
                        <div class="text-right d-print-none">
                            ...
                            {{--                                                               href="{{route('invoice.downloadInvoice', $invoiceItems->invoice_number)}}" target="_blank">--}}
